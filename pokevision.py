...
# List of target Pokémon names (Generation 2 starters)
...
# Define the neural network class (same as the one used during training)
class Net(nn.Module):
...
if __name__ == '__main__':
    ...
    csv_file = 'filtered_pokemon.csv'
    dataset = PokemonDataset(csv_file, transform=transform)
    ...
    # Create the dataset
    # Create the dataset
    test_size = int(0.2 * len(dataset))  # Use 20% of the dataset for testing
    train_size = len(dataset) - test_size  # The remaining 80% for training
    ...
    # Handles the loading and batching of test data
    testloader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
    classes = TARGET_POKEMON
